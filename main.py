...
from story import STORY, InteractionMap
...
class Game:
    ...
    def register_interaction(self, entity_id: str, response: str):
        ...
        if objective is not None:
            ...
            self.PENDING_STORY_ADVANCE = self.OBJECTIVE_COMPLETE and self._stage_requests_story_progression(self.get_current_story_stage())
...
class App:
                            ...
                            case pg.K_RETURN:
                                ...
                                if entity_id is not None:
                                    ...
                                    if interaction is not None:
                                        self.GAME.register_interaction(entity_id, interaction.get("response"))
    ...
    async def STATE_scene(self):
        ...
        icon_surface = None
        if self.GAME.INTERACTED_ENTITY_ICON is not None:
            icon_key = self.GAME.INTERACTED_ENTITY_ICON
            if icon_key is not None:
                icon_surface = pg.image.load(self.ASSETS.get(icon_key)).convert()
            ...
            for event in pg.event.get():
                match event.type:
                    ...
                    case pg.KEYDOWN:
                        match event.key:
                            case pg.K_RETURN:
                                if self.GAME.PENDING_STORY_ADVANCE:
                                    if self.GAME.progress_story():
                                        self.refresh_map_surface()
                                        self.NEXT.put(self.STATE_ingame)
                                    else:
